...
private class Compiler(private val p: Program, private val memWords: Int) {
  companion object {
  ...
  fun compile(): IntArray {
    // Top-level brk returns through the shared function epilogue; loops shadow this with their exit label.
    val lf = fresh()
    emitBlock(p.body, lf)
    mark(lf)
    emitReturn()
  ...
  private fun emitBlock(xs: List<Stmt>, breakLabel: String) =
    xs.forEach { emitStmt(it, breakLabel) }

  private fun emitStmt(s: Stmt, breakLabel: String) {
    when (s) {
      is Stmt.Assign -> {
        emitExpr(s.expr)
      ...
      is Stmt.Ife -> {
        ...
        emitBlock(s.elseBlk, breakLabel)
        emitJump(le)
        mark(lt)
        emitBlock(s.thenBlk, breakLabel)
        mark(le)
      }
      ...
      Stmt.Break -> emitJump(breakLabel)
    }
  }
